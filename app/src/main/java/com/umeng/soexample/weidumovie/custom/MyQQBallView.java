...
public class MyQQBallView extends View {
    private static final String TAG = "MyQQBallView";
    ...
    public MyQQBallView(Context context) {
        this(context, null);
    }

    public MyQQBallView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyQQBallView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN:
                getStartX = event.getX();
                getStartY = event.getY();
                setIsCanDrag(event);
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mIsCanDrag) {
                    if (isOutOfRang) {
                        ...
                        if (startX + 60 < endX || startY + 60 < endY || startX - 60 > endX || startY - 60 > endY) {
                            //消失动画
                            disappear = true;
                            if (onDragBallListener != null) {
                                onDragBallListener.onDisappear(endX, endY);
                            }
                            invalidate();
                        } else {
                            reset();
    ...
    public interface OnDragBallListener {
        void onDisappear(int endX, int endY);
    }
